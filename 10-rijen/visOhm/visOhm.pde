...
void draw(){
	...
	num = (sin(frameCount/300.1235)+1.982393)*4000;
	...
	for(int X = 0; X <= width;X += width){
		...
		for(int Y = 0; Y <= height;Y += height){
			...
			for(int i = (int)num ; i > 0 ; i-- ){
				...
				_x += (width/2.0-_x)/(sin(frameCount/30.0)+1.0);
...
if(dist(_x,_y,width/2,height/2)<sin(frameCount/300.0)*height/2-_size_of_dot){
					...
					fill(#FFFFFF);//lerpColor( #FFFFFF, c[(barvaCnt++)%c.length] , (sin((frameCount+1500)/3.0)+1.0)/2.0 )  );
