...
void setup() {
    ...
    pipeline = new GSPipeline(this, "v4l2src !  video/x-raw-yuv, width=640");  
...
void draw() {
    ...
    if (pipeline.available()) {
        ...
        int w = pipeline.width;
        int h = pipeline.height;

        /*for(int i = frameCount%10 ; i < pipeline.pixels.length;i+=5){//noise(frameCount/100.0)*30.0+4.0){
       
            float v = brightness(pipeline.pixels[i]);
            stroke(lerpColor(color(v),pipeline.pixels[i],noise(frameCount/300.0+i/30000.0)),noise(frameCount/3000.0)*noise(frameCount/40.0)*255);
            point(i%w/2+(1-noise(i/width/30.0,frameCount/30.0))*v/3.0+40,i/w/2+noise(frameCount/30.0)*5.0);
        
        
        }*/


        for(int i = 0 ; i < pipeline.pixels.length ; i++){

            stroke(pipeline.pixels[i]);
            point(i%(w+0.0),i/(h+0.0));
